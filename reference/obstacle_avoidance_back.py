...
l2=l1.split(' ')
#将数据转为矩阵，9600*6列，1行
m1=np.array(l2[0:9600*6])
#变形
m2=m1.reshape(9600,6)
m8 = m2[:,0:3]
m9 = np.array(m8)
m10=[]
...
for each in m9:
    ...
    #将其坐标换算到60*160坐标系下
    each_line[0] = int(i/160)
    each_line[1] = int(i%160)
    i = i+1
    #根据深度值是否在安全距离外，将其二值化
    if (each_line[2]==0 or (each_line[2]>safety_distance and each_line[2]<65500)):#65500是饱和点，距离小于200mm；距离值0是无穷远点，绝对安全。
        each_line[2] = 1
    else:
        each_line[2] = 0
...
Z = Z.reshape(60,160)
#将该矩阵保存到txt文件
np.savetxt('二值矩阵.txt', Z, fmt="%d", delimiter=' ')
