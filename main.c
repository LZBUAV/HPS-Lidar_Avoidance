...
int lidar_connect()
{
 ...
 int lidar_measure(uint16_t dist[])
 {  
    ret = HPS3D_SingleMeasurement(&handle);
    if(ret != RET_OK)
    {
        printf("SingleMeasurement failed\n");
        return 0;
    }
    else
    {
        //printf("measure success");
    }
    ...
    {
		//雷达倒装，所以倒序存放
        dist[9599-i] = handle.MeasureData.full_depth_data->distance[i];
    }
...
int uart_pack_send(char* rx_buffer,int len,int fd)
{
    int rx_length = write(fd, (void*)rx_buffer, len);
	...
	{
		printf("rx_length != sizeof(rx_buffer)");
		fflush(stdout);
	}
